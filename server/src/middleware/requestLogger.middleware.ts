...
const writeLog = (loglevel: LOG_LEVELS, message: string, requestId?: string) => {
  const logMsg = {
...
const logRequestToFileMW = logger({
  ...
  expressFormat: true,
  colorize: false,
  requestFilter: (req: Request, propName: string) => req[propName as keyof Request],
  responseFilter: (res: Response, propName: string) => res[propName as keyof Response],
  dynamicMeta(req) {
    return {
      product: "chat-app",
      ip: (req: Request) => req.headers['x-forwarded-for'] || req.socket.remoteAddress,
...
const requestLoggerMW = (req: Request, res: Response, next: NextFunction) => {
  ...
  req.id = xRequestId as string | undefined || `${Date.now()}-${randomUUID()}`;
