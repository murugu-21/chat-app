...
Deploy the `app/` directory to **Cloudflare Pages** (Git integration — deploys on
push to the configured branch). Build config:

- **Root directory:** `app`
- **Build command:** `npm run build`
- **Build output directory:** `dist` (relative to `app` → `app/dist`)

Environment variables on the Pages project (these are the names the SPA actually
reads; the API and websocket share **one** origin after the single-port merge):
...
`app/public/_redirects` (`/*  /index.html  200`) is bundled automatically so deep
links (`/auth/callback`, `/chat/:id`) serve the SPA. The production domain must match
the backend CORS origin (SP1) and the Cognito app client's callback/logout URLs.
